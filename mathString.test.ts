...
describe('test Regex for inputs', () => {
  ...
  it('select numbers', () => {
    expect(mathString.selectNumbers(input[0].replace(/\s*/g,''))).toEqual(['1', '+11', '+9', '-11e23'])
  })
  it('select parantesses and numbers between them', () => {
    expect(mathString.selectParantheses(input[1])).toEqual(['(1+1)', '(1111)', '(1*12/2/5-1+1)'])
  })
  it('select multiple or divide two numbers', () => {
    expect(JSON.stringify(mathString.selectMultiplicationOrDivisionePhrase(input[1]))).toEqual(JSON.stringify(['1*12']))
  })
})
describe('solve phrases math', () => {
  ...
  it('solve a phrase with parantheses', () => {
    ...
    expect(mathString.solve(input[8])).toBe('+44')
  })
})